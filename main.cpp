...
#define Load 42
#define Clear 43
#define background_white 44
#define background_black 45
#define background_red 46
...
void filling1stQuarter(HDC hdc,int xc, int yc, int R,bool is_Circle)
{
    ...
    else
    {
        ...
        while (x < y)
        {
            ...
            DrawLine1(hdc,xc,yc,xc+x,yc-y,c);// first
            DrawLine1(hdc,xc,yc,xc+y,yc-x,c);//first
        }
    }
}
void filling2ndQuarter(HDC hdc,int xc, int yc, int R,bool is_Circle)
{
    if(is_Circle)
    {
        for(int i=0; i< R; i++)
        {
            ...
            while (x < y)
            {
                ...
                SetPixel(hdc,xc+x,yc+y,c);//second
                SetPixel(hdc,xc+y,yc+x,c);//second
    ...
    else
    {
        ...
        while (x < y)
        {
            ...
            DrawLine1(hdc,xc,yc,xc+x,yc+y,c);//second
            DrawLine1(hdc,xc,yc,xc+y,yc+x,c);//second
        }
    }
...
void filling3rdQuarter(HDC hdc,int xc, int yc, int R,bool is_Circle)
{
    if(is_Circle)
    {
        for(int i=0; i< R; i++)
        {
            ...
            while (x < y)
            {
                ...
                SetPixel(hdc,xc-x,yc+y,c);//third
                SetPixel(hdc,xc-y,yc+x,c);// third
            }
        }
    }
    ...
    else
    {
        ...
        while (x < y)
        {
            ...
            DrawLine1(hdc,xc,yc,xc-x,yc+y,c);//third
            DrawLine1(hdc,xc,yc,xc-y,yc+x,c);// third
        }
    }
...
void NRMyFloodFill(HDC hdc,int x,int y,COLORREF Cb,COLORREF Cf)
{
    ...
    while(!S.empty())
    {
        ...
        S.push(Vector(v[0]+1,v[1]));
        S.push(Vector(v[0]-1,v[1]));
        S.push(Vector(v[0],v[1]+1));
        S.push(Vector(v[0],v[1]-1));
    }
}
//draw square
...
void drawBezier(HDC hdc,Vector& p1,Vector& p2,Vector& p3,Vector& p4,COLORREF c)
{
    ...
    {
        double drawX=pow(1-t,3)*p1[0]+3*t*pow(1-t,2)*p2[0]+3*t*t*(1-t)*p3[0]+t*t*t*p4[0];
        double drawY=pow(1-t,3)*p1[1]+3*t*pow(1-t,2)*p2[1]+3*t*t*(1-t)*p3[1]+t*t*t*p4[1];
...
void DrawLine_MidPoint(HDC hdc, int x1, int y1, int x2, int y2)
{
    ...
    if (abs(dy) <= abs(dx)) //sloope < 1
    {
        ...
        while(x<x2)
        {
            ...
            {
                d += d1 ;
                if ((dx < 0 && dy < 0) || (dx > 0 && dy > 0)) y++;//if sloope >0
                else y--; //sloop==0
            }
            x++;
            SetPixel(hdc, x, y, c);
...
{
    table *edgetable=new table[MAXENTRIES];
...
void GeneralPolygonFill(HDC hdc,point *polygon,int n,COLORREF c)
{
    ...
    int y=0;
    while(y<MAXENTRIES && table[y].size()==0)y++; //get the first not null index
...
{
    alldata += t + "," + to_string(x_1)+ "," +to_string(y_1) + "," + to_string(x_2) + "," + to_string(y_2)+ "," + to_string(c) + "\n";
}

void write_tofile_2(string t,int x_1,int y_1,int x_2,int y_2,int x_3,int y_3)
{
    alldata += t + "," + to_string(x_1)+ "," +to_string(y_1) + "," + to_string(x_2) + "," + to_string(y_2) + "," + to_string(x_3) + "," + to_string(y_3) +"," + to_string(c) + "\n";
}
...
write_tofile_4(string type,int num_quarter,int width,int height)
{
    alldata += type + "," + to_string(num_quarter)+ "," +to_string(width) + "," + to_string(height) +","+to_string(c) + "\n";
}
...
void Load_function(HDC hdc)
{
    ...
    if (file.is_open())
    {
        while ( getline (file,one_line) )
        {
            ...
            {
                int tem=1;
                int l1=stoi(vec[tem++]),l2=stoi(vec[tem++]),l3=stoi(vec[tem++]),l4=stoi(vec[tem++]),num1=stoi(vec[tem++]),num2=stoi(vec[tem++]);
                int num3=stoi(vec[tem++]),num4=stoi(vec[tem]);
                Vector s; s[0]=num1; s[1]=num2;
                drawRectangle_clipping(hdc,s,num3,num4,c);
            ...
            else if("Clipping_Square_Point" == vec[0])
            {
                ...
                Vector s;
                s[0]=stoi(vec[tem++]); s[1]=stoi(vec[tem++]);
            ...
            {
                int tem=1;
                int l3=stoi(vec[tem++]),l4=stoi(vec[tem++]),distance=stoi(vec[tem++]),xi=stoi(vec[tem++]),yi=stoi(vec[tem]);
            ...
            else if("Clipping_circle_Line" == vec[0])
            {
                ...
                int l2=stoi(vec[tem++]),l4=stoi(vec[tem++]),xi=stoi(vec[tem++]),yi=stoi(vec[tem++]),distance=stoi(vec[tem]);
                DrawCircleMidPoint(hdc,xi,yi,distance);
   ...
   // SetBkColor(hdc, RGB(12,34,210));
    ...
    case WM_RBUTTONDOWN:
        ...
        else if(type==3)//call Circle_Direct  fundtion
        {
            ...
            alldata+="Circle_Direct,"+to_string(xc1)+","+to_string(yc1)+","+to_string(radius)+","+to_string(c)+"\n";
        }
        else if(type==4)  //call Circle_Polar  fundtion
        {
            DrawCircle_Polar(hdc,xc1,yc1,radius);
            alldata+="Circle_Polar,"+to_string(xc1)+","+to_string(yc1)+","+to_string(radius)+","+to_string(c)+"\n";
        }
        else if(type==5) //call Circle_IterativePolar  fundtion
        {
            DrawCircle_Iterative(hdc,xc1,yc1,radius);
            alldata+="Circle_IterativePolar,"+to_string(xc1)+","+to_string(yc1)+","+to_string(radius)+","+to_string(c)+"\n";
        }
        else if(type==6) //call Circle_Midpoint fundtion
        {
            DrawCircle_MidPoint(hdc,xc1,yc1,radius);
            alldata+="Circle_Midpoint,"+to_string(xc1)+","+to_string(yc1)+","+to_string(radius)+","+to_string(c)+"\n";
        }
        else if(type==7) //call Circle_ModifiedMidpoint  fundtion
        {
            DrawCircle_ModifiedMidPoint(hdc,xc1,yc1,radius);
            alldata+="Circle_ModifiedMidpoint,"+to_string(xc1)+","+to_string(yc1)+","+to_string(radius)+","+to_string(c)+"\n";
        }
        ...
        else if(type==10)  //call Filling_Square    fundtion
        {
            ...
            else if(count2==1)
            {
                ...
                drawSquare(hdc,s,len,c);
                alldata += "Filling_Square,"+ to_string(s[0])+ "," + to_string(s[1])+ ","+ to_string(len) +"," + to_string(c) + "\n";
        ...
        else if(type==11)  //call Filling_Rectangle    fundtion
        {
            ...
            else if(count2==2)
            {
                ...
                drawRectangle(hdc,s,len,width,c);
                alldata += "Filling_Rectangle," + to_string(s[0])+ "," + to_string(s[1])+ ","+ to_string(len) + "," + to_string(width) +"," + to_string(c) + "\n";
        ...
        else if(type==19) //call Clipping_Rectangle_Point fundtion
        {
            ...
            else if(count2==3)
            {
                ...
                alldata+="Clipping_Rectangle_Point,"+to_string(l1)+","+to_string(l2)+","+to_string(s[0])+","+to_string(s[1])+","+to_string(len)+","+to_string(width)+","+to_string(c)+"\n";
                break;
            }
        ...
        else if(type==20) //call Clipping_Rectangle_Line  fundtion
        {
            ...
            else if(count2==4)
            {
                ...
                alldata+="Clipping_Rectangle_Line,"+to_string(l1)+","+to_string(l2)+","+to_string(l3)+","+to_string(l4)+","+to_string(s[0])+","+to_string(s[1])+","+to_string(len)+","+to_string(width)+","+to_string(c)+"\n";
                count2=3;
                break;
            }
        ...
        else if(type==21) //call Clipping_Rectangle_Polygon  fundtion
        {
            ...
            else if(count2==3)
            {
                ...
                if (index1 == NumberOfPoints - 1)
                {
                    ...
                    alldata+=to_string(s[0])+","+to_string(s[1])+","+to_string(len)+","+to_string(width)+","+to_string(c)+"\n";
                    index1 = 0;
                    enter=false;
                }
                else index1++;
        ...
        else if(type==22) //call Clipping_Square_Point   fundtion
        {
            ...
            else if(count2==2)
            {
                ...
                l2= HIWORD(lParam);
                PointClipping(hdc,l1,l2,s[0],s[1],s[0]+len,s[1]+len);
                alldata+="Clipping_Square_Point,"+to_string(l1)+","+to_string(l2)+","+to_string(s[0])+","+to_string(s[1])+","+to_string(len)+","+to_string(c)+"\n";
                break;
            }
        }
        else if(type==23) //call Clipping_Square_Line   fundtion
        {
            ...
            else if(count2==2)
            {
                ...
                alldata+="Clipping_Square_Line,"+to_string(l3)+","+to_string(l4)+","+to_string(xc1)+","+to_string(yc1)+","+to_string(s[0])+","+to_string(s[1])+","+to_string(len)+","+to_string(c)+"\n";
                //count2=2;
                break;
            }
        }
        else if(type==24) //call Clipping_circle_Point
        {
            ...
            else if(count2==2)
            {
                ...
                alldata+="Clipping_circle_Point,"+to_string(l3)+","+to_string(l4)+","+to_string(distance)+","+to_string(xc)+","+to_string(yc)+","+to_string(c)+"\n";
                break;
            }
        }
        else if(type==25) //call Clipping_circle_Line
        {
            ...
            else if(count2==3)
            {
                ...
                alldata+="Clipping_circle_Line,"+to_string(l1)+","+to_string(l3)+","+to_string(l2)+","+to_string(l4)+","+to_string(xc)+","+to_string(yc)+","+to_string(distance)+","+to_string(c)+"\n";
                count2=2;
                break;
            }
...
void AddMenus(HWND hwnd)
{
    ...
    for(int i=0; i<size; i++)
    {
        ...
        if(i!=size-1)  AppendMenu(Mouse_cursor,MF_SEPARATOR,NULL,NULL);
    }
    ...
    for(int i=0; i<size; i++)
    {
        ...
        if(i!=size-1)AppendMenu(Load_Store_Clear,MF_SEPARATOR,NULL,NULL);
    }
...
int APIENTRY WinMain(HINSTANCE hinst, HINSTANCE pinst, LPSTR cmd, int nsh)
{
    ...
    RegisterClass(&wc);
    HWND hwnd = CreateWindow("MyClass", "Graphic Project", WS_OVERLAPPEDWINDOW, 0, 0, 800, 600, NULL, NULL, hinst, 0);
    ShowWindow(hwnd, nsh);
    UpdateWindow(hwnd);
    MSG msg;
